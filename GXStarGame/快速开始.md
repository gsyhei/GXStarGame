...
### 基本操作
1. **点击任意水晶**：触发光效并选中
2. **点击相邻水晶**：若能形成 3 连，则交换成功
3. **形成 3 连以上**：自动消除，触发粒子与连锁动画
4. **连锁反应**：下落 & 新水晶生成后若再次匹配，会继续连锁

### 高级机制
- **爆炸水晶**：单次 4 连生成，触发十字范围消除
- **彩虹水晶**：单次 5 连（含以上）生成，清除整行整列
- **连锁倍率**：连续消除会提升 Combo 倍数与音效表现
- **魔法等级**：累计得分提升等级，进度条与徽章实时显示
- **暂停菜单**：点击右上角 `☰` 随时查看玩法或进入设置
- **玩法指南**：主菜单或暂停菜单可打开图文玩法说明页
